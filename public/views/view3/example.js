///// Karim Fanadka
//// not for use, only example for the json of the $scope.chunk
...
trackerApp.controller('view3Ctrl', function ($scope, $http, googleMapsAPIService, $mdDialog, $mdSidenav, dataBaseService, messages) {
    ...
    dataBaseService.getTrips().then(function (results) {
        $scope.trips = results;
        console.log('Client:: View3:: Fun run when load page :: list of trips: ' + results)
    });
    ...
    for ( var i = 0 ; i < 6 ; i++)
    {
        var jsonTrip = {title: "karim 1", description: "desc", expanded: false, content: {type: "type", quotes: "quotes"}};
        $scope.chunks[1] = jsonTrip;
    }

...
app.post('/addNewUser', function (request, response) {
    ...
    pg.connect(conString, function (err, client, done) {
        ...
        var query = client.query("INSERT INTO users(email, name, provider, provider_id, cell_number) values($1, $2, $3, $4, $5)",
            [request.body.email, request.body.name, 'facebook', request.body.id, '0500000000'], function (err, result) {
                //call `done()` to release the client back to the pool
                done();
